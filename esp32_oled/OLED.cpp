...
void OLED_DrawLine(u8 x1,u8 y1,u8 x2,u8 y2,u8 mode1)
{
  ...
  {
    OLED_DrawPoint(uRow,uCol,mode1);//画点
    xerr+=delta_x;
    yerr+=delta_y;
    if(xerr>distance)
    {
...
void OLED_ShowChar(u8 x,u8 y,u8 chr,u8 size1,u8 mode1)
{
  ...
  for(i=0;i<size2;i++)
  {
    ...
    for(m=0;m<8;m++)
    {
      if(temp&0x01)OLED_DrawPoint(x,y,mode1);
      else OLED_DrawPoint(x,y,!mode1);
      temp>>=1;
      y++;
    }
    x++;
...
void OLED_ShowString(u8 x,u8 y,const char  *chr,u8 size1,u8 mode1)
{
  ...
  {
    OLED_ShowChar(x,y,*chr,size1,mode1);
    if(size1==8)x+=6;
    else x+=size1/2;
    chr++;
  }
...
void OLED_ShowNum(u8 x,u8 y,u32 num,u8 len,u8 size1,u8 mode1)
{
  ...
  for(t=0;t<len;t++)
  {
    temp=(num/OLED_Pow(10,len-t-1))%10;
      if(temp==0)
      {
        OLED_ShowChar(x+(size1/2+m)*t,y,'0',size1,mode1);
      }
      else 
      {
        OLED_ShowChar(x+(size1/2+m)*t,y,temp+'0',size1,mode1);
      }
  }
}
...
void OLED_ScrollDisplay(u8 num,u8 space,u8 mode1)
{
  ...
  while(1)
  {
    if(m==0)
    {
      OLED_ShowChinese(64,56,t,16,mode1); //写入一个汉字保存在OLED_GRAM[][]数组中
      t++;
    }
    if(t==num)
      {
